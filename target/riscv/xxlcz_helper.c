...
target_ulong HELPER(xl_insert)(target_ulong a, target_ulong b,
                                target_ulong c, target_ulong d)
{
    uint32_t is2 = b & 0x1F;
    uint32_t is3 = c & 0x1F;
...
target_ulong HELPER(xl_addibne)(target_ulong rd, target_ulong rs1, uint32_t scale,
                                uint32_t imm, uint32_t pc)
{
    target_ulong val = pc;
    if ((rd + scale) != (rs1)) {
        if (imm <= 1024) {
