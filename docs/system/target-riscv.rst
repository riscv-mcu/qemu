...
of the MSTATUS ``MBE``/``SBE``/``UBE`` bits.

QEMU implements the data-endianness behaviour described by those bits. In
addition, the RISC-V CPU object exposes a ``big-endian`` boolean property
