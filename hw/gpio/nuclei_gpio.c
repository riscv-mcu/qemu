...
static void update_output_irq(NucleiGPIOState *s)
{
    uint32_t pending;
    uint32_t pin;
    ...
    for (int i = 0; i < s->ngpio; i++) {
        ...
        trace_nuclei_gpio_update_output_irq(i, (pending & pin) != 0);
    }
}

static void update_state(NucleiGPIOState *s)
{
    size_t i;
...
static uint64_t nuclei_gpio_read(void *opaque, hwaddr offset, unsigned int size)
{
    ...
    switch (offset) {
    case NUCLEI_GPIO_REG_VALUE:
        r = s->value;
        break;
    case NUCLEI_GPIO_REG_INPUT_EN:
        r = s->input_en;
        break;
    case NUCLEI_GPIO_REG_OUTPUT_EN:
        r = s->output_en;
        break;
    case NUCLEI_GPIO_REG_PORT:
        r = s->port;
        break;
    case NUCLEI_GPIO_REG_PUE:
        r = s->pue;
        break;
    case NUCLEI_GPIO_REG_DS:
        r = s->ds;
        break;
    case NUCLEI_GPIO_REG_RISE_IE:
        r = s->rise_ie;
        break;
    case NUCLEI_GPIO_REG_RISE_IP:
        r = s->rise_ip;
        break;
    case NUCLEI_GPIO_REG_FALL_IE:
        r = s->fall_ie;
        break;
    case NUCLEI_GPIO_REG_FALL_IP:
        r = s->fall_ip;
        break;
    case NUCLEI_GPIO_REG_HIGH_IE:
        r = s->high_ie;
        break;
    case NUCLEI_GPIO_REG_HIGH_IP:
        r = s->high_ip;
        break;
    case NUCLEI_GPIO_REG_LOW_IE:
        r = s->low_ie;
        break;
    case NUCLEI_GPIO_REG_LOW_IP:
        r = s->low_ip;
        break;
    case NUCLEI_GPIO_REG_IOF_EN:
        r = s->iof_en;
        break;
    case NUCLEI_GPIO_REG_IOF_SEL:
        r = s->iof_sel;
        break;
    case NUCLEI_GPIO_REG_OUT_XOR:
        r = s->out_xor;
        break;
    default:
    ...
    trace_nuclei_gpio_read(offset, r);
                              ...
                              uint64_t value, unsigned int size)
{
    ...
    trace_nuclei_gpio_write(offset, value);
...
static void nuclei_gpio_set(void *opaque, int line, int value)
{
    NucleiGPIOState *s = NUCLEI_GPIO(opaque);

    trace_nuclei_gpio_set(line, value);

    assert(line >= 0 && line < NUCLEI_GPIO_PINS);
...
static const VMStateDescription vmstate_nuclei_gpio = {
    .name = TYPE_NUCLEI_GPIO,
    .version_id = 1,
    .minimum_version_id = 1,
    .fields = (const VMStateField[]) {
        VMSTATE_UINT32(value,     NucleiGPIOState),
        VMSTATE_UINT32(input_en,  NucleiGPIOState),
        VMSTATE_UINT32(output_en, NucleiGPIOState),
        VMSTATE_UINT32(port,      NucleiGPIOState),
        VMSTATE_UINT32(pue,       NucleiGPIOState),
        VMSTATE_UINT32(rise_ie,   NucleiGPIOState),
        VMSTATE_UINT32(rise_ip,   NucleiGPIOState),
        VMSTATE_UINT32(fall_ie,   NucleiGPIOState),
        VMSTATE_UINT32(fall_ip,   NucleiGPIOState),
        VMSTATE_UINT32(high_ie,   NucleiGPIOState),
        VMSTATE_UINT32(high_ip,   NucleiGPIOState),
        VMSTATE_UINT32(low_ie,    NucleiGPIOState),
        VMSTATE_UINT32(low_ip,    NucleiGPIOState),
        VMSTATE_UINT32(iof_en,    NucleiGPIOState),
        VMSTATE_UINT32(iof_sel,   NucleiGPIOState),
        VMSTATE_UINT32(out_xor,   NucleiGPIOState),
        VMSTATE_UINT32(in,        NucleiGPIOState),
        VMSTATE_UINT32(in_mask,   NucleiGPIOState),
        VMSTATE_END_OF_LIST()
    }
};

static Property nuclei_gpio_properties[] = {
    DEFINE_PROP_UINT32("ngpio", NucleiGPIOState, ngpio, NUCLEI_GPIO_PINS),
    DEFINE_PROP_END_OF_LIST(),
};

static void nuclei_gpio_realize(DeviceState *dev, Error **errp)
{
    NucleiGPIOState *s = NUCLEI_GPIO(dev);
    ...
    qdev_init_gpio_in(DEVICE(s), nuclei_gpio_set, s->ngpio);
...
static void nuclei_gpio_class_init(ObjectClass *klass, void *data)
{
    ...
    device_class_set_props(dc, nuclei_gpio_properties);
    dc->vmsd = &vmstate_nuclei_gpio;
    dc->realize = nuclei_gpio_realize;
